...
def signUp():
    ...
    if signUpForm.is_submitted():
        ...
        repEmail = db_sess.query(User).filter(
            User.email == signUpForm.email.data).all()
        if len(ne) == 0 and len(repEmail) == 0:
            user = User(email=signUpForm.email.data,
                        ...
                        cityFrom=signUpForm.cityFrom.data)
        else:
            print("This nickname or email has already been creating")
            return render_template('signUp.html', title="Авторизация", form=signUpForm, message="Такой nickname или почта уже есть :)")
...
@app.route('/users_show/<int:userId>')
def showNostalgia(userId):
    
    userInfo = requests.get(
        f"http://localhost:5001/api/users/{userId}/map"
    )
    print(userInfo)
    try:
        ...
        print(userInfo)
        if not userInfo:
            return render_template('nostalgia.html', error=True, message="Bad id or error in request")
        
        return render_template('nostalgia.html', error=False,
                               user=userInfo["userNickname"], cityFrom=userInfo["userCity"], image=userInfo["map"])
    except Exception:
        return render_template('nostalgia.html', error=True, message="Bad id or error in request")
    
    # if not cityGeocode:
    #     message = "Error location"
    #     return render_template('nostalgia.html', message=message)
    ...
    # req = requests.get(
    #     f"{staticMapApi}ll={pos[0]},{pos[1]}&l=sat"
    # )
    ...
    # return render_template('nostalgia.html', user=currentUser, image='../../map.png')
