...
@blueprint.route('/api/jobs', methods=["PATCH"])
def editJob():
    if not request.json:
    ...
    if isinstance(request.json["id"], int) and request.json["id"] >= 1:
        ...
        jobWithEntryId = db_sess.query(Job).filter(Job.id == request.json["id"]).first()
        
        if jobWithEntryId is None:
            return jsonify({"error": "There is no incoming id in database"})
        
        jobWithEntryId.jobTitle = request.json["jobTitle"]
        jobWithEntryId.teamLeaderId = request.json["teamLeaderId"]
        jobWithEntryId.workSize = request.json["workSize"]
        jobWithEntryId.collaborators = request.json["collaborators"]
        jobWithEntryId.isFinished = request.json["isFinished"]
        jobWithEntryId.nameOfCreator = request.json["nameOfCreator"]
